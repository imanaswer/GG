...
import { useWorkshop, useCancelWorkshop } from "@/hooks/useData";
...
export default function WorkshopDetail({ params }: { params: Promise<{ id: string }> }) {
      ...
      <>
        ...
        <main style={{ background: "#050505", minHeight: "100vh", paddingTop: 120 }}>
          <div className="container-lg">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 320px", gap: 32 }} className="workshop-grid">
  ...
  const sideInfo = [
    { Icon: DollarSign, label: "Price",       value: workshop.priceDisplay, sub: workshop.price === 0 ? "Free" : "Per participant", emphasis: true },
    { Icon: Calendar,   label: "Dates",       value: dateRange,            sub: null as string | null,           emphasis: false },
    { Icon: Clock,      label: "Session",     value: workshop.sessionType === "series" ? `${workshop.sessionCount} sessions` : "Single session", sub: workshop.sessionDuration, emphasis: false },
    { Icon: Target,     label: "Level",       value: workshop.skillLevel,   sub: null as string | null,           emphasis: false },
    { Icon: Users,      label: "Audience",    value: audienceLabel,         sub: workshop.ageGroup || null,        emphasis: false },
    { Icon: MapPin,     label: "Location",    value: workshop.address || workshop.location, sub: workshop.distance ? `${workshop.distance} away` : null, emphasis: false },
  ];
    ...
    <>
      ...
      <main style={{ background: "#050505", color: "#fff", minHeight: "100vh", position: "relative", overflow: "hidden" }}>
            ...
            <Reveal>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32, flexWrap: "wrap", gap: 12 }}>
                <Link href="/workshops" style={{
                  ...
                  <ArrowLeft size={12} /> All workshops
                </Link>
                <button
                  onClick={handleShare}
                  style={{
        ...
        <section style={{ paddingBottom: 120 }}>
          <div className="container-lg">
            <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 340px", gap: 32 }} className="workshop-grid">
              {/* Left */}
              <div>
                {/* Enrollment status */}
                <Reveal>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
                      <div>
                        ...
                        <p style={{ fontSize: 14, color: "rgba(255,255,255,0.65)" }}>
                          <strong style={{ color: "#fff", fontWeight: 700 }}>{workshop.participants}</strong> enrolled &middot; {spotsLeft} spots left
                        </p>
                      </div>
                      <div style={{
                ...
                <Reveal>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 24 }}>
                    ...
                    <TabButton id="sessions"   active={tab} onClick={setTab} label="Sessions" count={workshop.sessions.length} />
                    <TabButton id="instructor" active={tab} onClick={setTab} label="Instructor" />
                    <TabButton id="reviews"    active={tab} onClick={setTab} label="Reviews"  count={workshop.testimonials.length} />
                  </div>
                </Reveal>
                ...
                {tab === "sessions" && (
                  <Reveal>
                    <div style={{
                      ...
                      border: "1px solid rgba(255,255,255,0.06)", borderRadius: 20, padding: "24px 28px",
                    }}>
                        ...
                        {workshop.sessions.map((s, i) => (
                          <div key={i} style={{
                            ...
                            <div style={{ flex: 1 }}>
                              ...
                              {s.description && <p style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", lineHeight: 1.6 }}>{s.description}</p>}
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  <Reveal>
                    <div style={{
                      ...
                      <div style={{ display: "flex", alignItems: "flex-start", gap: 20 }}>
                        ...
                        <div>
                          <h3 style={{ fontSize: 20, fontWeight: 800, color: "#fff", marginBottom: 8 }}>{workshop.instructor.name}</h3>
                          ...
                          {workshop.instructor.bio && (
                            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", lineHeight: 1.75, marginTop: 12 }}>
                              {workshop.instructor.bio}
                            </p>
                          )}
                        </div>
              ...
              <aside>
                <div style={{
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        <div style={{
                          ...
                          <p style={{ fontSize: 12, color: "rgba(255,255,255,0.55)" }}>
                            {workshop.userRegistration?.registrationType === "youth"
                              ? `${workshop.userRegistration?.participantName} is enrolled for this workshop`
                  ...
                  <div style={{
                    ...
                    >
                      <Phone size={12} /> {workshop.organizerContact}
                    </a>
                  </div>
                </div>
              </aside>
          ...
          >
            ...
            >
              ...
              <form onSubmit={handleSubmit}>
                {/* Audience type selector — only for "all" */}
                {workshop.audienceType === "all" && (
                  <div style={{ marginBottom: 16 }}>
                    <label className="eyebrow" style={{ margin: "0 0 10px", display: "block" }}>Registering for</label>
                    <div style={{ display: "flex", gap: 10 }}>
                      {[
                        { v: "adult" as const, l: "Myself" },
                        { v: "youth" as const, l: "A child" },
                      ].map(opt => (
                        <button key={opt.v} type="button"
                          onClick={() => {
                            setRegType(opt.v);
                            setParticipantName(opt.v === "adult" ? (user?.name ?? "") : "");
                            setParticipantAge("");
                          }}
                          style={{
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 7, marginBottom: 16 }}>
                  <label className="eyebrow" style={{ margin: 0 }}>
                    {regType === "youth" ? "Child\u2019s full name" : "Your name"}
                  </label>
                  <input
                    placeholder={regType === "youth" ? "e.g. Arjun Kumar" : "e.g. Rahul Sharma"}
                    value={participantName}
                    onChange={e => setParticipantName(e.target.value)}
                    required
                    disabled={regType === "adult" && workshop.audienceType !== "all"}
                    style={{
                      height: 44, padding: "0 14px", borderRadius: 12,
                      border: "1px solid rgba(255,255,255,0.08)",
                      background: (regType === "adult" && workshop.audienceType !== "all")
